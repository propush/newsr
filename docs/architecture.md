...
10. Pressing `B` in provider home opens the brief setup screen when no refresh preflight or article pipeline is active. Successful generation closes setup and opens a full-size brief reader screen. Brief reports include numbered article references; typing a number and pressing `Enter` opens the referenced article in a transient article reader with export, view-mode, URL, More Info, and article-Q&A actions. `Esc` returns from that transient article reader to the brief, and closing the brief reader returns to provider home.

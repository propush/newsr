...
- `newsr.yml`: user config for refresh limits, default provider update schedule, LLM endpoint/model settings, translation language, UI locale (`en` or `ru`), `[ALL]` provider-home visibility (`ui.show-all = true|false`), provider-home sorting (`ui.provider_sort.primary = unread|name`, `ui.provider_sort.direction = asc|desc`), and export quality

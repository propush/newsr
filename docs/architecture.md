...
- `newsr.config`: config dataclasses plus loading and validation for `newsr.yml`, including UI locale and provider-home sort settings
...
4. Startup prunes expired articles, restores the selected theme from global options, restores the `[ALL]` reader scope, loads cached articles, and opens the provider home as the app's home screen with `[ALL]` plus enabled providers sorted by `ui.provider_sort`.
...
- `newsr.yml`: user config for refresh limits, LLM endpoint/model settings, translation language, UI locale (`en` or `ru`), provider-home sorting (`ui.provider_sort.primary = unread|name`, `ui.provider_sort.direction = asc|desc`), and export quality

...
- Brief review opens from provider home, collects period and source options in a setup screen, then shows the generated report with contributor article counts in a full-size reader screen. Generated brief reports are transient UI content.
...
10. Pressing `B` in provider home opens the brief setup screen. Successful generation closes setup and opens a full-size brief reader screen; closing that reader returns to provider home.
...
12. Reader state such as the current article, view mode, and scroll offset is persisted per scope, while the selected theme is stored separately in the single-row options table.

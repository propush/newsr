...
- Article Q&A keeps a temporary per-modal transcript, combines stored article text with live DuckDuckGo results, and lets the input recall submitted questions with `Up` and `Down`. The transcript and question history are cleared when the modal closes or changes article and are not persisted.
...
10. Pressing `B` in provider home opens the brief setup screen when no refresh preflight or article pipeline is active. The setup screen displays the localized current generation step while article selection, summary compression, repair retries, final synthesis, fallback, and read-state updates run. Successful generation closes setup and opens a full-size brief reader screen. Brief reports include validated numbered article references; compression and final synthesis repair invalid markers, uncited selected articles, and source-free statements with bounded retries before the reader opens. Typing a number and pressing `Enter` opens the referenced article in a transient article reader with export, view-mode, URL, More Info, and article-Q&A actions. `Esc` returns from that transient article reader to the brief, and closing the brief reader returns to provider home.
11. The source manager can refresh an HTTP provider catalog through `discover_targets()`, persist replacement target lists while preserving still-valid selections, edit per-provider cron overrides, and delete watched-topic providers.
12. Reader state such as the current article, view mode, and scroll offset is persisted per scope, while the selected theme is stored separately in the single-row options table. Brief article jumps keep their article selection, view mode, and scroll position in transient UI state and do not update scoped reader-state rows.
13. "More info", article Q&A, open-link confirmation, browser handoff, and export actions run outside the refresh pipeline but reuse the same stored article content, provider registry, DuckDuckGo search client, and configured LLM endpoint. The shared DuckDuckGo and LLM clients retry transient transport failures within the configured request budget while preserving cancellation. Export actions render the active reader view, so original mode uses the stored source title and body. "More info" results are cached per article in SQLite, while article-Q&A transcripts and input history remain modal-local and are discarded when the screen closes.
...
- `cache/newsr-llm.log`: request log for LLM calls, transient transport retry metadata, refresh candidate deduplication, brief-generation stage metadata, repair retry metadata, and non-provider network request metadata such as DuckDuckGo search URLs, methods, statuses, and errors

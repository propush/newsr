...
4. Startup prunes expired articles, restores the selected theme from global options, restores the `[ALL]` reader scope, loads cached articles, and opens a provider home screen with `[ALL]` plus enabled providers sorted by `ui.provider_sort`.

...
12. "More info", article Q&A, open-link confirmation, browser handoff, and export actions run outside the refresh pipeline but reuse the same stored article content, provider registry, DuckDuckGo search client, and configured LLM endpoint. Export actions render the active reader view, so original mode uses the stored source title and body. "More info" results are cached per article in SQLite, while article-Q&A transcripts remain modal-local and are discarded when the screen closes.

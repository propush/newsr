...
from .urls import CANARYMEDIA_ROOT, normalize_target_path


class CanaryMediaProvider:
    provider_id = "canarymedia"
    display_name = "Canary Media"
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(target.payload.get("path", f"/articles/{target.target_key}"))
        html = self._read_url(f"{CANARYMEDIA_ROOT}{target_path}", cancellation)

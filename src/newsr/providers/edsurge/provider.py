...
from .urls import EDSURGE_ROOT, normalize_target_path


class EdSurgeProvider:
    provider_id = "edsurge"
    display_name = "EdSurge"
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(target.payload.get("path", f"/news/{target.target_key}"))
        html = self._read_url(f"{EDSURGE_ROOT}{target_path}", cancellation)

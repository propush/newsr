...
from .urls import HBR_ROOT, normalize_target_path
...
class HBRProvider:
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(
            target.payload.get("path", f"/topic/subject/{target.target_key}")
        )
        html = self._read_url(f"{HBR_ROOT}{target_path}", cancellation)

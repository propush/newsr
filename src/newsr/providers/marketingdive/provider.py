...
from .urls import MARKETINGDIVE_ROOT, normalize_target_path


class MarketingDiveProvider:
    provider_id = "marketingdive"
    display_name = "Marketing Dive"
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(target.payload.get("path", f"/topic/{target.target_key}/"))
        html = self._read_url(f"{MARKETINGDIVE_ROOT}{target_path}", cancellation)

...
from .urls import SCIENCEDAILY_ROOT, normalize_target_path


class ScienceDailyProvider:
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(
            target.payload.get("path", f"/news/{target.target_key}/")
        )
        html = self._read_url(f"{SCIENCEDAILY_ROOT}{target_path}", cancellation)

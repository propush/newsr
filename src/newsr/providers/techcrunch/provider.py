...
from .urls import TECHCRUNCH_ROOT, normalize_target_path


class TechCrunchProvider:
    ...
    def default_targets(self) -> list[ProviderTarget]:
        return [
            ...
            for option in BASE_TOPIC_OPTIONS
        ]
    ...
    def fetch_candidates(
        ...
        target_path = normalize_target_path(target.payload.get("path", f"/{target.target_key}/"))
        html = self._read_url(f"{TECHCRUNCH_ROOT}{target_path}", cancellation)
    ...
    def _target_from_option(self, option: TopicOption, *, selected: bool) -> ProviderTarget:

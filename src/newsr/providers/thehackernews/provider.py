...
from .urls import THN_ROOT


class TheHackerNewsProvider:
    ...
    def default_targets(self) -> list[ProviderTarget]:
        return [
            ...
            for option in BASE_SECTION_OPTIONS
        ]
    ...
    def fetch_candidates(
        ...
        section_path = target.payload.get("path", f"/{target.target_key}")
        html = self._read_url(f"{THN_ROOT}{section_path}", cancellation)
    ...
    def _target_from_option(self, option: SectionOption, *, selected: bool) -> ProviderTarget:
        return ProviderTarget(
            provider_id=self.provider_id,
            target_key=option.slug,
            target_kind="category",
            label=option.label,
            payload={"path": option.path},
            selected=selected,
        )
